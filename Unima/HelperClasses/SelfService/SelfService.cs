...
public class SelfService : ISelfService
{
    ...
    public async Task<bool> LogIn(string username, string password)
    {
        try
        {
            using HttpRequestMessage request = ConfigHttpRequest(HttpMethod.Get, "https://selfservice.birjand.ac.ir");
            ...
            using HttpRequestMessage loginRequest = ConfigHttpRequest(HttpMethod.Post, response.RequestMessage.RequestUri.AbsoluteUri,
                loginCookies, $"idsrv.xsrf={bodyXsrf}&username={username}&password={password}", "application/x-www-form-urlencoded");

            using HttpResponseMessage loginResponse = await _httpClient.SendAsync(loginRequest);
            ...
            using HttpRequestMessage finalRequest = ConfigHttpRequest(HttpMethod.Post, "https://selfservice.birjand.ac.ir",
                loginCookies,
                dataStr.ToString(),
    ...
    {
        using HttpRequestMessage balanceRequest = ConfigHttpRequest(HttpMethod.Get, "https://selfservice.birjand.ac.ir/api/v0/Credit", _neededCookies);
        using HttpResponseMessage balanceResponse = await _httpClient.SendAsync(balanceRequest);
