...
public class ApplicationIdentityErrorDescriber : IdentityErrorDescriber
{
    ...
    public override IdentityError InvalidToken() => new IdentityError { Code = nameof(InvalidToken), Description = $"کد وارد شده معتبر نمی‌باشد" };

    public override IdentityError PasswordMismatch() => new IdentityError { Code = nameof(PasswordMismatch), Description = $"رمز عبور فعلی وارد شده معتبر نمی‌باشد" };
}